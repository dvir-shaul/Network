...
#define source_ip "10.2.0.4"
#define destination_ip "16.217.10.4"
  
unsigned short in_cksum(unsigned short *buf, int len);
void send_raw_ip_packet(struct sniff_ip* ip);

//this program sends a fake massege to a spesific destination
...
int main() { 
	...
	ip->ip_ttl = 64;
	ip->ip_src.s_addr = inet_addr(destination_ip);
	ip->ip_dst.s_addr = inet_addr(source_ip);
...
//this function send a spoofed packet to a destination who havent sent a request
void send_raw_ip_packet(struct sniff_ip* ip)
{
